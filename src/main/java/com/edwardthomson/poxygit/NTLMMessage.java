...
public abstract class NTLMMessage
{
	...
	public static int checkHeader(byte[] message) throws Exception
	{
		...
		if (message[0] != 0x4e || message[1] != 0x54 || message[2] != 0x4c || message[3] != 0x4d ||
	...
	protected static short getInt16(byte[] message, int position)
	{
		...
		return (short) ((message[position] & 0xFF) | ((message[position + 1] & 0xFF) << 8));
	}
	...
	protected static int getInt32(byte[] message, int position)
	{
		...
		return (message[position] & 0xFF) | ((message[position + 1] & 0xFF) << 8) |
	...
	static class Type2Message extends NTLMMessage
	{
		...
		public Type2Message(int flags, byte[] challenge, String target, TargetInformation targetInfo, Version version)
		{
			...
			final Charset charset = ((flags & FLAG_NEGOTIATE_UNICODE) == FLAG_NEGOTIATE_UNICODE) ?
	...
	static class TargetInformation
	{
		...
		{
			byte[] domainName = domain != null ? createTargetInformationName(TARGET_INFO_DOMAIN, domain, charset) :
					new byte[0];
			byte[] serverName = server != null ? createTargetInformationName(TARGET_INFO_SERVER, server, charset) :
					new byte[0];
			byte[] dnsDomainName = dnsDomain != null ?
					createTargetInformationName(TARGET_INFO_DOMAIN_DNS, dnsDomain, charset) :
					new byte[0];
			byte[] dnsServerName = dnsServer != null ?
					createTargetInformationName(TARGET_INFO_SERVER_DNS, dnsServer, charset) :
					new byte[0];
			...
			byte[] targetInfo = new byte[domainName.length + serverName.length + dnsDomainName.length +
					dnsServerName.length + timestampValue.length + end.length];
