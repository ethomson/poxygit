...
public class GetOptions
{
	...
	public void parse(String[] args) throws OptionException
	{
		...
		for (int i = 0; i < args.length; i++)
		{
			...
			/* An argument */
			else if (doneWithOptions == false && ((args[i].startsWith("--") && args[i].length() > 2) ||
					(args[i].startsWith("-") && args[i].length() > 1)))
			{
				Option thisOption = (args[i].startsWith("--")) ? longArgs.get(args[i].substring(2)) :
						shortArgs.get(args[i].charAt(1));
